...
Component({
    ...
    pickerShow: {
      ...
      observer: function(val) { // 弹出动画
        ...
        if (val) {
          ...
          const animationOpacity = wx.createAnimation({
            duration: 500,
            timingFunction: 'ease',
          });
          setTimeout(() => {
  ...
  methods: {
    ...
    readConfig() {
      ...
      const limitStartTimeArr = formatTime(limitStartTime);
      const limitEndTimeArr = formatTime(limitEndTime);
    ...
    onConfirm: function() {
      ...
      const startTime = new Date(this.data.startPickTime.replace(/-/g, '/'));
      const endTime = new Date(this.data.endPickTime.replace(/-/g, '/'));
      if (startTime <= endTime || !this.data.endDate) {
        this.setData({
          startTime,
          endTime,
        });
        const startArr = formatTime(startTime).arr;
        const endArr = formatTime(endTime).arr;
    ...
    compareTime(val, type) {
      const h = val[3] ? this.data.HourList[val[3]] : '00';
      const m = val[4] ? this.data.MinuteList[val[4]] : '00';
      const s = val[5] ? this.data.SecondList[val[5]] : '00';
      const time =
      ...
      const start = this.data.limitStartTime;
      const end = this.data.limitEndTime;
      const timeNum = new Date(time.replace(/-/g, '/')).getTime();
      let year; let month; let day; let hour; let min; let sec; let limitDate;
      const tempArr = [];
    ...
    setStartDate: function(year, month, day, hour, minute, second) {
      const pickerDateArr = this.setPickerDateArr(
          'start',
          year,
          month,
          day,
