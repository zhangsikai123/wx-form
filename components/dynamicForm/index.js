...
Component({
  ...
  methods: {
    // 表单初始化
    formInit() {
      const pickerMap = {}; const fileMap = {}; const inputMap = {}; const dateMap = {};// 存储各表单变化后的值,表单id为索引
      const pickers = []; const files = []; const inputs = []; const datePickers = [];
    ...
    formSubmit() {
      ...
      for (const i in this.data) { // 获取表单数据后缀为Map
        if (i.match(/Map$/)) {
          formData = Object.assign({}, formData, this.data[i]);
        }
      }
      let hasError = false;
      for (const i in formData) {// 循环验证所有表单数据规则
        const info = formData[i];
    ...
    onFileRead(e) {
      console.log(e);
      for (const val of e.detail.file) {
    ...
    bindUserInfo: function(e) {
      ...
      if (detail.iv) {
        http.post('/user/bindinfo', {
          ...
          return getApp().getUserInfo().then((userInfo) => {
            this.setData({
              userInfo: userInfo,
            });
          });
